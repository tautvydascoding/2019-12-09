...
?>
        ...
        <div class="container-fluid">
          ...
          <!-- ******* MAIN SECTION ********* -->
          <main>
            <h1 class="ml-5 mb-4">Order details</h1>
            <?php
              ...
              if (count($_SESSION["items"]) > 0) {
                  ...
                  echo "<div class='row ml-5 mb-4 d-flex align-items-center' itemId='$item[id]'>
                          <div class='col'>
                            <div class='row text-center align-items-center'>
                              ...
                              <p class='col-2 price'>$item[price] EUR</p>
                              <p class='col'> Qty: $itemQty</p>
                              <p class='col total-item'>$sum EUR</p>
                            </div>
                          </div>
                        </div>";
                }
                echo "<hr><div class='row d-flex justify-content-end total'>
                ...
                if (isset($_COOKIE["logedin"])) {
                  $user = getUser($_COOKIE['logedin']);
                  echo "<h3 class='ml-5'>Hello member $user[user_name]!</h3>
                        <h4 class='ml-5'>Please check shipping details</h4>
                        <form class='ml-5' action='../controller/generate-order.php' method='post'>
                          <input class='mb-3' type='text' name='name' value='$user[name]' required><br>
                          <input class='mb-3' type='text' name='lname' value='$user[lname]' required><br>
                          <input class='mb-3' type='text' name='email' value='$user[email]' required><br>
                          <input class='mb-3' type='text' name='adress' value='$user[adress]' required><br>
                          <input class='mb-3' type='text' name='postCode' value='$user[post_code]' required><br>
                          <div class='d-flex justify-content-end'>
                            <input class='mb-3 btn btn-lg' type='submit' name='' value='Confirm Order' required><br>
                          </div>
                        </form>";
                } else {
                  echo "<div class='row'>
                          <div class='col'>
                            <h3 class='row ml-5 mb-3'>Please login to buy as a member</h3>
                            <input class='row mb-2 ml-5' type='text' name='username' value='' placeholder='username'>
                            <input class='row mb-2 ml-5' type='password' name='pass' value='' placeholder='password'>
                            <input class='row login-btn btn btn-outline-secondary ml-5' type='submit' name='' value='Login'>
                            <h5 class='row mb-2 ml-5 login-failed'>Wrong username or password</h5>
                            <h3 class='row mt-5 ml-5 mb-3'>Buy as a guest</h3>
                            <form class='ml-5' action='../controller/generate-order.php' method='post'>
                              <input class='mb-3' type='text' name='name' value='' placeholder='Name' required><br>
                              <input class='mb-3' type='text' name='lname' value='' placeholder='Last name' required><br>
                              <input class='mb-3' type='text' name='email' value='' placeholder='email' required><br>
                              <input class='mb-3' type='text' name='adress' value='' placeholder='Adress' required><br>
                              <input class='mb-3' type='text' name='postCode' value='' placeholder='Postal code' required><br>
                              <div class='d-flex justify-content-end'>
                                <input class='mb-3 btn btn-lg' type='submit' name='' value='Confirm Order' required><br>
                              </div>
                            </form>
